...
class Model:
    def __init__(self, is_training, config):
        ...
        with tf.variable_scope("coverage"):
            u_f = tf.get_variable(
                "u_f",
                [config.num_keywords * config.word_embedding_size, config.num_keywords],
            )
            res1 = tf.sigmoid(
            ...
            for time_step in range(num_steps):
                            ...
                            tf.tanh(
                                tf.add(
                                    tf.add(
                                        ...
                                        tf.matmul(keyword_inputs[:, s2, :], w2),
                                    ),
                                    b,
                                )
                            ),
                ...
                with tf.variable_scope("RNN_sentence"):
                    ...
                    (cell_output, state) = cell(
                        tf.concat([inputs[:, time_step, :], mt], axis=1), state
                    )
...
def run_epoch(
    ...
    if flag == 0:
        prob, _state, _last_output, _last_gate, weight, _phi, _ = session.run(
            [
                m._prob,
                m.final_state,
                m.end_output,
        ...
        prob, _state, _last_output, _last_gate, weight, _ = session.run(
            [
                m._prob,
                m.final_state,
                m.end_output,
...
def main(_):
    with tf.Graph().as_default(), tf.Session(config=config_tf) as session:
        ...
        initializer = tf.random_uniform_initializer(
            -gen_config.init_scale, gen_config.init_scale
        )
        ...
        print("model loading ...")
        model_saver.restore(
            session, gen_config.model_dir + "model--%d" % gen_config.save_time
        )
        print("Done!")

        test_word = ["体会", "母亲", "滴水之恩", "母爱", "养育之恩"]
        ...
        prob, _state, _last_output, _last_gate, weight, _phi = run_epoch(
            session,
            mtest,
            test_data,
            tf.no_op(),
        ...
        if gen_config.is_sample:
            try:
                top_indices = np.random.choice(
                    gen_config.vocab_size, beam_size, replace=False, p=prob.reshape(-1)
                )
            except:
                top_indices = np.random.choice(
                    gen_config.vocab_size, beam_size, replace=True, p=prob.reshape(-1)
                )
        else:
        ...
        for _ in range(len_of_sample - 1):
            beam_candidates = []
            for b in beams:
                test_data = np.int32(b[2])
                prob, _state, _last_output, _last_gate, weight = run_epoch(
                    session,
                    mtest,
                    test_data,
                    tf.no_op(),
