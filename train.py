    ...
    def __init__(self, is_training, config, filename):
        ...
        features = tf.parse_single_example(
            ...
            features={
                ...
                "key_words": tf.FixedLenFeature(
                    [batch_size * config.num_keywords], tf.int64
                ),
            },
        )
        ...
        with tf.device("/cpu:0"):
            ...
            )  # tensor shape(batch_size, num_steps, size)
...
def run_epoch(session, m, eval_op):
    ...
    for step in range(total_step + 1):
        ...
        cost, _ = session.run(
            [m.cost, eval_op], {m.initial_state: state, m._init_output: initial_output}
        )
        ...
        if step and step % (total_step // 5) == 0:
            print(
                "%d-step perplexity: %.3f cost-time: %.2f s"
                % (step, np.exp(costs / iters), time.time() - start_time)
            )
            start_time = time.time()
    ...
    with tf.Graph().as_default(), tf.Session(config=config_tf) as session:
        ...
        for i in range(config.max_max_epoch):
            ...
            if (i + 1) % config.save_freq == 0:
                print("model saving ...")
                model_saver.save(session, config.model_dir + "model--%d" % (i + 1))
                print("Done!")
